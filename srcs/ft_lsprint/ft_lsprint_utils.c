...
/*   Updated: 2018/01/10 17:57:09 by kdumarai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_spaces_for_width_fd(int fd, size_t len, t_opts opts)
{
	size_t	n;

	n = (len < opts.width) ? opts.width - len : 0;
	while (n--)
		ft_putchar_fd(' ', fd);
}
...
{
	if (!opts.justf)
		print_spaces_for_width_fd(fd, ft_nbrlen((long long)n), opts);
	ft_putnbr_fd(n, fd);
	if (opts.justf)
		print_spaces_for_width_fd(fd, ft_nbrlen((long long)n), opts);
}
...
{
	if (!opts.justf)
		print_spaces_for_width_fd(fd, ft_nbrlen(n), opts);
...
void		print_str_width_fd(int fd, char *s, t_opts opts)
{
	...
	if (!opts.justf)
		print_spaces_for_width_fd(fd, slen, opts);
...
{
	if (!opts.justf)
		print_spaces_for_width_fd(fd, 1, opts);
	ft_putchar_fd(c, fd);
	if (opts.justf)
		print_spaces_for_width_fd(fd, 1, opts);
}
